...
param_values_no_fd <- param_values[!(names(param_values) %in% c("fd"))]          

# Defining "param_names" with the updated values 
param_names <- c(names(param_values_no_fd), names(init_values))

# Changing parameter values
param_values_no_fd["NF"] <- 0.3           # Sandfly to human ratio (allowed values: >=0) 
param_values_no_fd["season_amp"] <- 1      # Relative amplitude of seasonality (allowed values: 0->1) 
param_values_no_fd["season_t"] <- 8 / 12   # The particular (8th currently) month of 12 months of the year 

start_time <- 0
sim_duration <- 100 # years

# Set time-varying parameters representing changes 
policy_fd <- covariate_table(fd = c(0, 1 , 1),
                          order = "constant",
...
pars_det <- c(param_values_no_fd, init_values)
...
pars_sto <- c(param_values_no_fd, init_values_sto)
...
# Print to disk
ggsave(filename = sprintf("NF%.2f-betaH%.2f-foi_ext%.3f_det_vs_stoch_over_time_fd.pdf",
                          param_values_no_fd["NF"],
                          param_values_no_fd["betaH"],
                          param_values_no_fd["foi_ext"]),
